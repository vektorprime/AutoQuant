...
def _quantize_one_layer_q4k_mixed(layer: nn.Linear) -> dict:
    """Q4_K with mixed 3/4-bit per superblock based on range + LS refinement."""
    W = layer.weight.data.float()
    out_features, in_features = W.shape
    ...
    blk_range = w_blk_max - w_blk_min
    threshold = blk_range.median()
    mask_3bit = blk_range < threshold

    d = torch.where(mask_3bit, blk_range / 7.0, blk_range / 15.0)
    d[d < 1e-8] = 1e-8
    dmin = w_blk_min.abs().clamp(min=1e-8)

    d_sub = torch.where(mask_3bit.unsqueeze(-1), (w_sub_max - w_sub_min) / 7.0,
                        (w_sub_max - w_sub_min) / 15.0)
    d_sub[d_sub < 1e-8] = 1e-8
    ...
    maxq = mask_3bit.unsqueeze(-1).unsqueeze(-1) * 7.0 + (~mask_3bit).unsqueeze(-1).unsqueeze(-1) * 15.0
    q = torch.round(
        (W_r + eff_offset.unsqueeze(-1)) / eff_scale.unsqueeze(-1)
    )
    q = torch.clamp(q, 0, 15).to(torch.uint8)
    q = torch.where(mask_3bit.unsqueeze(-1).unsqueeze(-1), torch.clamp(q, 0, 7), q)
    ...
    for name, data in tqdm(meta.items(), desc="Saving compressed"):
        ...
        if data.get("format") == "q4_k":
            ...
            np.savez(fname,
                     ...
                     format=data["format"])
        elif data.get("format") == "q4_k_mixed":
            quants = data["quants"]
            q_out = _pack_4bit(quants)
            sm_packed = _pack_q4k_sm(data["scales_6bit"], data["mins_6bit"])
            d8 = _pack_fp16_to_log8(data["d"])
            dm8 = _pack_fp16_to_log8(data["dmin"])
            mask = data["mask_3bit"]
            mask_packed = np.packbits(mask.reshape(-1), bitorder='little')
            np.savez(fname,
                     quants=q_out,
                     d8=d8,
                     dm8=dm8,
                     scales_mins_packed=sm_packed,
                     mask_3bit=mask_packed,
        ...
        if fmt == "q4_k":
            ...
            if layer.weight.shape[1] % QK_K != 0:
                logger.warning("Skipping %s: in_features %d not divisible by %d",
                               name, layer.weight.shape[1], QK_K)
                continue
            meta[name] = _quantize_one_layer_q4k_mixed(layer)
        else:
...
def parse_args():
    ...
    parser.add_argument("--format", default="q2_kmeans",
                        choices=["q2_kmeans", "q4_k", "q4_k_mixed"],
