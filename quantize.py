...
def decode_q4k(packed: dict, dtype: torch.dtype = torch.float32) -> torch.Tensor:
    ...
    if sm_last == 9 and sm_bits_scale == 5 and sm_bits_min == 4:
        ...
        min_ref = min_ref | (s[:, :, 1] & 0x07)

        sc_norm = torch.zeros(out_features, n_blocks, 8)
        min_norm = torch.zeros(out_features, n_blocks, 8)
        sc_norm[:, :, 0] = scale_ref.float() / 63.0
        min_norm[:, :, 0] = min_ref.float() / 31.0
        ...
        mn_bytes = sm[:, :, 6:11].to(dtype=torch.int32)
        sc_norm = torch.zeros(out_features, n_blocks, 8)
        ...
        packed_40 = mn_bytes[:, :, 0].long()
        packed_40 |= mn_bytes[:, :, 1].long() << 8
        packed_40 |= mn_bytes[:, :, 2].long() << 16
        packed_40 |= mn_bytes[:, :, 3].long() << 24
        packed_40 |= mn_bytes[:, :, 4].long() << 32
        min_norm = torch.zeros(out_features, n_blocks, 8)
        for i in range(8):
...
def _pack_values_6bit(values_8: torch.Tensor) -> torch.Tensor:
    """Pack 8 6-bit values (..., 8) into 6 bytes (..., 6)."""
    v = values_8.to(torch.int32)
    packed = torch.zeros(*values_8.shape[:-1], 6, dtype=torch.uint8)
    packed[..., 0] = (v[..., 0] | (v[..., 1] << 6)) & 0xFF
    packed[..., 1] = ((v[..., 1] >> 2) | (v[..., 2] << 4)) & 0xFF
    packed[..., 2] = ((v[..., 2] >> 4) | (v[..., 3] << 2)) & 0xFF
    packed[..., 3] = (v[..., 4] | (v[..., 5] << 6)) & 0xFF
    packed[..., 4] = ((v[..., 5] >> 2) | (v[..., 6] << 4)) & 0xFF
    packed[..., 5] = ((v[..., 6] >> 4) | (v[..., 7] << 2)) & 0xFF
    return packed


def _pack_values_5bit(values_8: torch.Tensor) -> torch.Tensor:
    """Pack 8 5-bit values (..., 8) into 5 bytes (..., 5)."""
    v = values_8.to(torch.int64)
    packed_40 = v[..., 0]
    for i in range(1, 8):
        packed_40 = packed_40 | (v[..., i] << (i * 5))
...
def _delta_encode_scales_mins_9byte(
    ...
    sf = sn[:, :, 0:1].copy()
    mf = mn[:, :, 0:1].copy()
    sd_signed = np.clip(sn[:, :, 1:] - sf, -8, 7)
    md_signed = np.clip(mn[:, :, 1:] - mf, -8, 7)

    sd_u = (sd_signed + 8).astype(np.uint8)
    md_u = (md_signed + 8).astype(np.uint8)

    ps = np.zeros_like(sn)
    pm = np.zeros_like(mn)
    ps[:, :, 0:1] = sf
    pm[:, :, 0:1] = mf
    ps[:, :, 1:] = np.clip(sf + sd_signed, 1, 63).astype(np.uint8)
    pm[:, :, 1:] = np.clip(mf + md_signed, 0, 31).astype(np.uint8)

    packed = np.zeros((out_f, n_b, 9), dtype=np.uint8)
    sf_i = sf[..., 0].astype(np.int64)
    mf_i = mf[..., 0].astype(np.int64)
    sd_i = sd_u.astype(np.int64)
    md_i = md_u.astype(np.int64)

    packed[..., 0] = (sf_i & 0x3F) | ((mf_i >> 3) & 0x03) << 6
    for i in range(7):
        bi = i + 1
        lo = (mf_i & 0x07) if i == 0 else (md_i[..., i - 1] & 0x07)
        packed[..., bi] = (
            lo
            | (sd_i[..., i] << 3)
            | (((md_i[..., i] >> 3) & 0x01) << 7)
    ...
    sm_packed = torch.from_numpy(packed.copy())
    perturbed_scales = torch.from_numpy(ps.copy()).to(torch.uint8)
    perturbed_mins = torch.from_numpy(pm.copy()).to(torch.uint8)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Delta-encode scales and mins within each superblock using "
            "sub-block 0 as reference plus 4-bit signed deltas. Packs into "
